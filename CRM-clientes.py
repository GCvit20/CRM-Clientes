...
def nuevo_cliente():
    ...
    guardar.grid(row=3, column=1, pady=10)
...
def eliminar_cliente():
    ...
    respuesta = messagebox.askokcancel('Eliminar', 'Estas seguro que quieres eliminar el cliente ' + cliente[1] + '?')
...
btn_agregar = Button(root, text='Nuevo Cliente', command=nuevo_cliente) #Creamos el boton
btn_agregar.grid(column=0,row=0) #agregamos el boton

btn_eliminar = Button(root, text='Eliminar Cliente', command=eliminar_cliente) #Creamos el boton
btn_eliminar.grid(column=1,row=0) #agregamos el boton
